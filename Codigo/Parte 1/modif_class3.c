 ...
 * Returns: (none)
 * Side-Effects: modifies st_texto pointer
 *
 * Description:
 *
 *****************************************************************************/
...
void PreencheTabelaPalavras ( char *ficheiro, st_texto *t )
{
  ...
  while ( ( palavra = LePalavra ( f ) ) != NULL ) {
    if ( ( n = NovaPalavra ( palavra, &(*t) ) ) == -1 )	{
      (*t).palavras[(*t).n_dist_palavras] = (char*) malloc(
...
void EscreveFicheiro ( char *ficheiro, st_texto *t )
{
  ...
  nome =  (char*) malloc(sizeof(char) * (strlen(ficheiro) + strlen("palavras.txt") + 1));
  if ( nome == NULL ) {
    fprintf ( stderr, "ERROr: not enough memory available!\n" );
    exit ( 5 );
  }
